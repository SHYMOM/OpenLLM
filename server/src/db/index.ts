...
const DB_PATH = path.resolve(__dirname, '../../data/openllm.db');
...
export function getAdminPassword(): string {
  // Priority: 1. DB, 2. ENV, 3. Default 'admin'
  const dbPass = getSetting('admin_password');
  if (dbPass) return dbPass;
  
  const envPass = process.env.ADMIN_PASSWORD;
  if (envPass && envPass.trim() !== '') return envPass;
...
function migrateModelsV9(db: Database.Database) {
  ...
  // Cleanup old unified API key prefix if it exists
  const row = db.prepare("SELECT value FROM settings WHERE key = 'unified_api_key'").get() as { value: string } | undefined;
  if (row && row.value.startsWith('OpenLLM-')) {
    const newVal = row.value.replace('OpenLLM-', 'openllm-');
    db.prepare("UPDATE settings SET value = ? WHERE key = 'unified_api_key'").run(newVal);
    console.log('Migrated unified API key prefix to openllm-');
  }
}

/**
 * Add meta column to chat_messages if it doesn't exist
 */
function migrateMessagesV1(db: Database.Database) {
  try {
    db.prepare('ALTER TABLE chat_messages ADD COLUMN meta TEXT').run();
  } catch (e: any) {
    // Ignore if column already exists
    if (!e.message.includes('duplicate column name')) {
      console.error('Migration error (meta column):', e);
    }
  }
}

/**
 * V10 (May 2026): Remove confirmed-dead OpenRouter :free models.
 * - inclusionai/ling-2.6-1t:free → 404 "no longer available as a free model"
 * - liquid/lfm-2.5-1.2b-instruct:free → 404
 * - liquid/lfm-2.5-1.2b-thinking:free → 404
 * - poolside/laguna-xs.2:free → 404
 * Also disable nvidia/nemotron-3-nano-30b-a3b:free (404 "No endpoints found")
 */
function migrateModelsV10(db: Database.Database) {
  ...
  const apply = db.transaction(() => {
    ...
    // Disable rather than delete (might come back)
    db.prepare("UPDATE models SET enabled = 0 WHERE platform = 'openrouter' AND model_id = 'nvidia/nemotron-3-nano-30b-a3b:free'").run();
  });
  apply();
}
...
function ensureUnifiedKey(db: Database.Database) {
  ...
  if (!existing) {
    const key = `openllm-${crypto.randomBytes(24).toString('hex')}`;
