...
import { z } from 'zod';
import type { ChatMessage } from '@openllm/shared/types.js';
import { routeRequest, recordRateLimitHit, recordSuccess, type RouteResult } from '../services/router.js';
import { recordRequest, recordTokens, setCooldown } from '../services/ratelimit.js';
import { getDb, getUnifiedApiKey, getSetting } from '../db/index.js';

/**
 * Auto-disable a model in the DB when the provider confirms it's dead (404).
 * This prevents the router from trying it again on subsequent requests.
 */
function autoDisableModel(modelDbId: number, reason: string) {
  try {
    const db = getDb();
    db.prepare('UPDATE models SET enabled = 0 WHERE id = ?').run(modelDbId);
    console.log(`[AutoDisable] Model DB ID ${modelDbId} disabled: ${reason}`);
  } catch (e) {
    console.error('[AutoDisable] Failed to disable model:', e);
  }
}
...
/** Check if the error indicates the model itself is permanently dead (not a transient issue). */
function isModelDeadError(err: any): boolean {
  const msg = (err.message ?? '').toLowerCase();
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (isRetryableError(err)) {
        // If the model is permanently dead (404), auto-disable it in the DB
        if (isModelDeadError(err)) {
          autoDisableModel(route.modelDbId, err.message);
        }
